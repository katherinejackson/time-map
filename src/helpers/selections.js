...
export const getShapeSelectionOptions = (shape) => {
    if (shape === shapes.SPIRAL.id) {
        return {
            ...
            spiralWidth: { name: 'Spiral Width', values: [5, 10, 15, 20, 25, 30, 35] },
            spiralTightness: { name: 'Spiral Tightness', values: [0.01, 0.015, 0.02, 0.025, 0.03] }
        }
    } else if (shape === shapes.ROW.id) {
        return {
            dayWidth: { name: 'Day Width', values: [0.125, 0.25, 0.5] },
            rowHeight: { name: 'Row Height', values: [15, 20, 25, 30, 35, 40] },
