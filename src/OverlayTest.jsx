...
const Overlay = () => {
    ...
    const [topCorner, setTopCorner] = useState({
        mapBoundsX: map.getPixelBounds().min.x, 
        mapBoundsY: map.getPixelBounds().min.y,
        offsetX: 0,
        offsetY: 0,
    })
    ...
    useEffect(() => {
        if (pg) {
            redrawOverlay(topCorner.offsetX, topCorner.offsetY)
        }
...
useEffect(() => {
    if (p5 && pg) {
        map.off('drag')
        map.on('drag', () => {
            const newX = map.getPixelBounds().min.x
...
const mouseMoved = (p5) => {
    ...
    locationClusters.forEach((cluster, index) => {
        if (Math.abs(p5.mouseX - (cluster.x + topCorner.offsetX)) < cluster.minDistanceX && Math.abs(p5.mouseY - (cluster.y + topCorner.offsetY) + pinAdjustment) < cluster.minDistanceY) {
            setHover(index)
            hoverFound = true
        }
    })
